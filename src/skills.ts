...
export async function loadSkills(extraDirectories: string[]) {
  ...
  async function readText(path: string) {
    ...
    if (!stat.isFile())
      throw new Error(`Skill inputs must be regular files: ${path}`);
    if (stat.size > 1024 * 1024)
      throw new Error(`Skill file exceeds 1 MiB: ${path}`);
    ...
    async function walk(relative: string) {
      for (const entry of await readdir(join(directory, relative), {
        ...
        if (entry.isSymbolicLink())
          throw new Error(`Skill symlinks are not supported: ${path}`);
