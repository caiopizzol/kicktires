...
import { mkdtemp, writeFile, rm, readFile, readlink, symlink } from "node:fs/promises";
...
test("pins committed revisions and excludes dirty working-tree files", async () => {
  ...
  try {
    ...
    await expect(readFile(join(out, "head", "DANGLING.md"))).rejects.toThrow("ENOENT");
    ...
    expect((await readFile(join(out, "head.tar"))).includes(Buffer.from("._source.txt"))).toBe(
      false,
    );
    expect(await readlink(join(out, "head", "AGENTS.md"))).toBe("source.txt");
    expect(await readFile(join(out, "head", "AGENTS.md"), "utf8")).toBe("head\n");
    expect(await readFile(join(out, "head", "version.txt"), "utf8")).toBe("$Format:%H$\n");
    expect(snapshot.head).toBe(head);
    expect(await readFile(join(out, "head", "source.txt"), "utf8")).toBe("head\n");
    expect(await readFile(join(repo, "source.txt"), "utf8")).toBe("dirty secret\n");
    await symlink("/etc/passwd", join(repo, "link"));
    git("add", "link");
    git("commit", "-qm", "link");
    await expect(snapshotRepository(repo, base, "HEAD", join(root, "unsafe"))).rejects.toThrow(
      "Unsupported repository symlink",
    );
  } finally {
...
test("rejects paths that escape the snapshot or Git metadata", () => {
  for (const path of ["/absolute", "../parent", "x/../parent", ".git/config", "x\\file", ""])
