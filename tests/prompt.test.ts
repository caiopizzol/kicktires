...
test("project instructions reach the review request separately from context", () => {
  ...
  expect(message).toContain("Choose relevant investigation tools");
});

test("profiles without custom instructions retain the standard review request", () => {
  const message = reviewMessage();
  expect(message).toBe(
    "Review /workspace/review.json and /workspace/change.diff. Follow the built-in review instructions and load supplied skills when relevant. Choose relevant investigation tools; configured checks and browser access are available when useful. Cite tool call IDs as evidenceRefs. Additional user context: None supplied.",
  );
  expect(message).toContain("Choose relevant investigation tools");
